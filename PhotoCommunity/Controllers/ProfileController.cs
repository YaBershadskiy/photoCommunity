    ...
    public class ProfileController : Controller
    {
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return RedirectToAction("Edit", new { Message = ManageMessageId.ChangePasswordSuccess });
            }
            AddErrors(result);
            return View(model);
        }
        ...
        public ActionResult Edit(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
               message == ManageMessageId.ChangePasswordSuccess ? "Ваш пароль изменен."
               : message == ManageMessageId.EditProfileSuccess ? "Профиль успешно изменён."
               : message == ManageMessageId.SetPasswordSuccess ? "Пароль задан."
               : message == ManageMessageId.SetTwoFactorSuccess ? "Настроен поставщик двухфакторной проверки подлинности."
               : message == ManageMessageId.Error ? "Произошла ошибка."
               : message == ManageMessageId.AddPhoneSuccess ? "Ваш номер телефона добавлен."
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(ApplicationUser model, HttpPostedFileBase uploadImage)
        {            
            if (ModelState.IsValid)
            {               
